...
export default function HomeFeed() {
  ...
  useEffect(() => { let cancelled = false; async function load() { const [{ data }, { data: ratingData }] = await Promise.all([supabase.from("gigs").select("id, title, description, category, price_ngn, is_ai_assisted, gallery_image_paths, created_at, seller_profiles ( display_name, verification_status )").eq("status", "active").order("created_at", { ascending: false }).limit(24), supabase.rpc("get_gig_ratings")]); if (cancelled) return; setGigs((data as unknown as GigRow[]) ?? []); const map: Record<string, CardRating> = {}; ((ratingData as Array<{ gig_id: string } & CardRating>) ?? []).forEach((row) => { map[row.gig_id] = row; }); setRatings(map); setLoading(false); } load(); return () => { cancelled = true; }; }, []);
