...
// Renders one of two pre-built shaders (from the 21st.dev Shader Builder
// specs) as a fullscreen WebGL1 background. A single triangle covers the
// whole viewport — one draw call per frame, no libraries, no dependency on
// the network or Supabase. Capped at devicePixelRatio 2 and paused when the
// tab isn't visible, so it stays cheap on phones.

type ShaderVariant = "metaballs" | "silk";
...
const FRAGMENT_SRC: Record<ShaderVariant, string> = {
  metaballs: SHARED_HEADER + METABALLS_SHADE + SHARED_MAIN,
  silk: SHARED_HEADER + SILK_SHADE + SHARED_MAIN,
};
...
> = {
  metaballs: {
    ...
    timeMultiplier: 1.33,
    shape: [1.32, 0.58, 0.5, 0.0],
    surface: [2.4, 0.94, 0.0, 1.0],
    finish: [0.0, 0.0, 0.0, 0.05],
    transform: [2068.0, 0.0, 0.0, 0.0],
  },
  silk: {
    colors: ["#02010A", "#04052E", "#3D2C8D", "#916BBF"],
    timeMultiplier: 0.76,
    shape: [1.26, 0.28, 0.5, 0.0],
    surface: [2.4, 1.11, 0.0, 1.0],
    finish: [0.0, 0.0, 0.0, 0.05],
    transform: [1581.0, 0.0, 0.0, 0.0],
  },
};
...
export default function ShaderBackground({
  ...
  useEffect(() => {
    ...
    if (!gl) return; // no WebGL support — canvas just stays empty, page still works
    ...
    gl.attachShader(program, compile(gl.FRAGMENT_SHADER, FRAGMENT_SRC[variant]));
    ...
    const colorArray = new Float32Array(24);
    ...
    function frame() {
      if (!running) return;
      resize();
      const t = ((performance.now() - start) / 1000) * cfg.timeMultiplier;
      gl!.uniform3fv(uColors, colorArray);
      gl!.uniform4f(uScene, canvas!.width, canvas!.height, t, 4.0);
      gl!.uniform4f(uShape, cfg.shape[0], cfg.shape[1], cfg.shape[2], cfg.shape[3]);
      gl!.uniform4f(uSurface, cfg.surface[0], cfg.surface[1], cfg.surface[2], cfg.surface[3]);
      gl!.uniform4f(uFinish, cfg.finish[0], cfg.finish[1], cfg.finish[2], cfg.finish[3]);
      gl!.uniform4f(uTransform, cfg.transform[0], cfg.transform[1], cfg.transform[2], cfg.transform[3]);
      gl!.uniform4f(uSpace, 0, 0, 0, 0);
      gl!.uniform4f(uCursor, 0, 2.0, 0.65, 0.46); // presence 0 — cursor is off
